...
Board::Board()
{
	CaseRect = new IntRect[24];
	CaseSprite = new Sprite[24];
...
void Board::InitialiserCase(Texture &texture, FILE* file)
{
    int x, y, i = 0;
    while (fscanf_s(file, "%d %d\n", &x, &y) == 2) { // Vérification du retour de fscanf_s pour la lecture
        std::cout << "(" << x << "," << y << ")" << std::endl;
        CaseRect[i].left = x;
        CaseRect[i].top = y;
        CaseRect[i].width = 37;
        CaseRect[i].height = 37;
        CaseSprite[i].setTexture(texture);
        CaseSprite[i].setTextureRect(CaseRect[i]);
        CaseSprite[i].setPosition(x, y);

        i++;
    }
}

Sprite& Board::GetSpriteCase(int i)
{
	return CaseSprite[i];
}
